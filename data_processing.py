...
#                     timer_state = step.get('TimerState', {}).get('TimerStats', {})
...
#                         'BytesInUse': memory_state.get('BytesInUse', 0),
#                         'MaxBytesInUse': memory_state.get('MaxBytesInUse', 0),
#                         'BytesFromOS': memory_state.get('BytesFromOS', 0),
...
#                         'Number of Fully Loaded Parts': assembly_state.get('NumFullyLoadedParts', 0),
#                         'Number of Partially Loaded Parts': assembly_state.get('NumPartiallyLoadedParts', 0),
#                         'Number of Minimally Loaded Parts': assembly_state.get('NumMinimallyLoadedParts', 0),
#                         'Number of OD Parts': assembly_state.get('NumOODParts', 0),

#                         'Time required to perform an operation': timer_state.get('Time (seconds)', 0),
        ...
        if filename.endswith('.json'):
            ...
            with open(file_path, 'r') as file:
                ...
                for step in steps:
                    step_id = step.get('StepId', '')
                    memory_state = step.get('MemoryState', {})
                    assembly_state = step.get('AssemblyState', {}).get('StructureDetails', {})
                    timer_state = step.get('TimerState', {}).get('TimerStats', {})
...
def create_plot(df, columns, ylabel, title):
    ...
    if not existing_columns:
        return "No valid columns found for plotting."
    ...
    group_width = bar_width * len(steps) + bar_spacing * (len(steps) - 1) + group_spacing
    group_positions = np.arange(len(baselines)) * group_width
    colors = plt.cm.tab10(np.linspace(0, 1, len(existing_columns)))
    ...
    for i, step in enumerate(steps):
        ...
        for j, column in enumerate(existing_columns):
            ax.bar(bar_positions,
                   step_data[column],
                   bar_width,
                   bottom=bottoms,
...
def create_memory_usage_plot(df):
    columns = [
 'BytesInUse', 'MaxBytesInUse', 'BytesFromOS',
    ]
    return create_plot(
...
def create_assembly_state_plot(df):
    columns = [
        ...
        'Number of Minimally Loaded Parts', 'Number of OD Parts'
    ]
    return create_plot(
...
def generate_stepwise_memory_usage_plots(df, selected_steps):
    memory_columns = [ 'BytesInUse', 'MaxBytesInUse', 'BytesFromOS']
    stepwise_plots = {}
...
def generate_stepwise_assembly_state_plots(df, selected_steps):
    assembly_columns = ['Number of Fully Loaded Parts', 'Number of Partially Loaded Parts', 'Number of Minimally Loaded Parts','Number of OD Parts' ]
    stepwise_plots = {}
    for step in selected_steps:
        step_data = df[df['StepId'] == step]
        fig, ax = plt.subplots(figsize=(10, 5))
...
def generate_assembly_state_line_plot(df):
    assembly_columns = [
        'Number of Fully Loaded Parts', 'Number of Partially Loaded Parts', 'Number of Minimally Loaded Parts','Number of OD Parts'
    ]
    fig, ax = plt.subplots(figsize=(14, 10))
    for column in assembly_columns:
        sns.lineplot(data=df, x='StepId', y=column, marker='o', linewidth=2, label=column, ax=ax)
